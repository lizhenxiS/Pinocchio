...
#define OUT { vertices.clear(); edges.clear(); return; }
	...
	cout << "vertice : " << verts << endl;

    if(verts == 0)
        return;
    
    fixDupFaces();
	if (verts != vertices.size())
		system("pause");
	cout << "vertice : " << vertices.size() << endl;
	cout << "Succes to fixDupFaces !" << endl;

    computeTopology();
	if (verts != vertices.size())
		system("pause");
	cout << "vertice : " << vertices.size() << endl;
	cout << "Success to computeTopology ! " << endl;

    computeVertexNormals();
	if (verts != vertices.size())
		system("pause");
	cout << "vertice : " << vertices.size() << endl;
	cout << "Success to computeVertexNormals ! " << endl;
...
void Mesh::computeTopology()
{
    ...
    for(i = 0; i < (int)edges.size(); ++i) {
        ...
        if(halfEdgeMap[v1].count(v2)) {
			cout << "Error: duplicate edge detected: " << endl;
...
{
	VertexNeighbor.clear();

    int i;
    int lineNum = 0;
	vector<int> temp_nonsence;
    while(!strm.eof()) {
        ++lineNum;
        ...
        if(words[0][0] == 'v') {
			...
			VertexNeighbor.push_back(temp_nonsence);
        ...
        if(words[0][0] == 'f') {
			...
			//对于a[0]点
			//已有邻居数
			int temp_round = VertexNeighbor[a[0]-1].size();
			bool nb1 = false;
			bool nb2 = false;
			bool nb0 = false;
			//遍历，若不存在当前邻居，则加入
			for (int i = 0; i < temp_round; i++)
			{
				if ((a[1]-1) == VertexNeighbor[a[0]-1][i])
					nb1 = true;
				if ((a[2]-1) == VertexNeighbor[a[0]-1][i])
					nb2 = true;
			}
			if (!nb1 && !nb2)
			{
				VertexNeighbor[a[0]-1].resize(temp_round + 2);
				VertexNeighbor[a[0]-1][temp_round] = a[1] - 1;
				VertexNeighbor[a[0]-1][temp_round + 1] = a[2] - 1;
			}
			else
			{
				if (!nb1)
				{
					VertexNeighbor[a[0]-1].resize(temp_round + 1);
					VertexNeighbor[a[0] - 1][temp_round] = a[1] - 1;
				}
				else
					if (!nb2)
					{
						VertexNeighbor[a[0] - 1].resize(temp_round + 1);
						VertexNeighbor[a[0] - 1][temp_round] = a[2] - 1;
					}
			}
			
			//对于a[1]点
			temp_round = VertexNeighbor[a[1] - 1].size();
			nb0 = false;
			nb1 = false;
			nb2 = false;
			for (int i = 0; i < temp_round; i++)
			{
				if ((a[0] - 1) == VertexNeighbor[a[1] - 1][i])
					nb0 = true;
				if ((a[2] - 1) == VertexNeighbor[a[1] - 1][i])
					nb2 = true;
			}
			if (!nb0 && !nb2)
			{
				VertexNeighbor[a[1] - 1].resize(temp_round + 2);
				VertexNeighbor[a[1] - 1][temp_round] = a[0] - 1;
				VertexNeighbor[a[1] - 1][temp_round + 1] = a[2] - 1;
			}
			else
			{
				if (!nb0)
				{
					VertexNeighbor[a[1] - 1].resize(temp_round + 1);
					VertexNeighbor[a[1] - 1][temp_round] = a[0] - 1;
				}
				else
				{
					...
					{
						VertexNeighbor[a[1] - 1].resize(temp_round + 1);
						VertexNeighbor[a[1] - 1][temp_round] = a[2] - 1;
					}
				}
			}

			//对于点a[2]
			temp_round = VertexNeighbor[a[2] - 1].size();
			nb0 = false;
			nb1 = false;
			nb2 = false;
			for (int i = 0; i < temp_round; i++)
			{
				if ((a[0] - 1) == VertexNeighbor[a[2] - 1][i])
					nb0 = true;
				if ((a[1] - 1) == VertexNeighbor[a[2] - 1][i])
					nb1 = true;
			}
			if (!nb0 && !nb1)
			{
				VertexNeighbor[a[2] - 1].resize(temp_round + 2);
				VertexNeighbor[a[2] - 1][temp_round] = a[0] - 1;
				VertexNeighbor[a[2] - 1][temp_round + 1] = a[1] - 1;
			}
			else
			{
				if (!nb0)
				{
					VertexNeighbor[a[2] - 1].resize(temp_round + 1);
					VertexNeighbor[a[2] - 1][temp_round] = a[0] - 1;
				}
				else
				{
					...
					{
						VertexNeighbor[a[2] - 1].resize(temp_round + 1);
						VertexNeighbor[a[2] - 1][temp_round] = a[1] - 1;
					}
				}
			}
        }
	...
	cout << "read obj successed ! And successed in finding Nerghbor !" << endl;
	cout << "VertexNeighbor 's size :" << VertexNeighbor.size() << "  VertexNeighbor[0] 's size :" << VertexNeighbor[0].size() << endl;
}
